...
export function PaymentProgress({
  ...
  const { priceCents, paidCents, state } = summary
  const tone = state === "PAID" ? "bg-emerald-400" : "bg-amber-400"
  const reference = Math.max(priceCents, paidCents) || 1

  return (
    <div
      className="flex h-2 w-full gap-px overflow-hidden rounded-full bg-white/[0.07]"
      role="img"
      aria-label={`Cobrado ${formatMoney(paidCents)} de ${formatMoney(priceCents)}`}
    >
      {payments.map((payment) => (
        <div
          key={payment.id}
          className={`h-full ${tone} transition-[width,background-color] duration-500 ease-out`}
...
export function PaymentAccountHeader({
  ...
  const { paidCents, priceCents, dueCents, overpaidCents, state, installments } = summary
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-xs">
        <span className="text-zinc-400">
          {installments === 0
            ...
            : installments === 1
              ? "1 pago registrado"
              : `${installments} pagos registrados`}
        </span>
        {unpriced ? (
          <span className="text-zinc-400">Todavía sin precio acordado</span>
        ) : dueCents > 0 ? (
    ...
    <ol className="relative space-y-px border-l border-white/10 pl-4">
      {payments.map((payment) => (
        <li key={payment.id} className="relative flex flex-wrap items-baseline justify-between gap-x-4 gap-y-0.5 py-2.5">
          <span
            className="absolute -left-[1.3125rem] top-[1.0625rem] h-1.5 w-1.5 rounded-full bg-emerald-400 ring-2 ring-[#0c0c0e]"
            aria-hidden
          />
          <div className="min-w-0">
            <p className="text-sm text-zinc-200">
              {formatPaymentDate(payment.paidAt)}
              <span className="text-zinc-400"> · {paymentMethodLabels[payment.method] ?? payment.method}</span>
            </p>
            {payment.note && <p className="truncate text-xs text-zinc-400">{payment.note}</p>}
          </div>
          <span className="shrink-0 text-sm font-medium tabular-nums text-emerald-300">
