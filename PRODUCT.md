...
- **Cadencia del barrido:** el plan Hobby de Vercel sólo permite crons diarios, así que el de `vercel.json` es una red de seguridad, no el reloj real. La puntualidad la da un servicio de ping externo (cron-job.org) que llama a `/api/publish-reminders/run` cada 15 minutos con el `CRON_SECRET` en la cabecera. Un webhook no sirve acá: no hay evento externo que avise, el evento es que pasó el tiempo.
