...
- Las imágenes se encogen al subirlas si pasan de **8 MB**, que es el techo común: Instagram admite 8 y Facebook 10. Por debajo no se toca el archivo. El caso que lo motivó fue real: tres fotos de 8.44, 10.04 y 9.91 MB salieron en Instagram y Facebook rechazó el lote entero por la de 10.04, cuarenta kilobytes por encima de su límite. Aceptar un archivo que una red publica y la otra no es prometer algo que no se puede cumplir.
...
- Las piezas de tipo **historia** salen solas en Instagram (`media_type=STORIES`). Historia o feed no se deduce del archivo —la misma foto sirve para las dos—, así que manda el tipo elegido al planificar.
- Una historia es **un solo archivo**. Con varios no se publica el primero en silencio: eso perdería los otros sin que nadie se entere. Se dice, y la salida es una pieza por historia, que además es como se planifican.
- Las historias van **sin texto**: Instagram no admite pie de foto y mandarlo hace fallar la llamada. El copy de la pieza se conserva igual, para publicarla a mano o reutilizarlo. El panel lo avisa antes de programar, o quien escriba un copy va a esperar que salga.
- En Facebook la historia de foto son dos pasos —subir sin publicar y convertirla— y la de video pide la subida por tramos, otro protocolo entero: ésa se rechaza con un mensaje que dice qué hacer en vez de fallar con un error de Meta que no explica nada.
...
- La pestaña **Resultados** cierra el círculo: planifiqué esto, salió tal día, rindió así. Trae de Meta alcance, me gusta, comentarios, guardados, compartidos y reproducciones de cada publicación, y los suma para el mes.
- Regla que ordena toda esa pantalla: **nunca un cero inventado**. Un dato que no vino se guarda en `null` y se muestra como «—». Enseñarle a un cliente un alcance de cero porque faltaba un permiso es peor que no enseñarle nada, y por eso las columnas admiten nulo en vez de tener `DEFAULT 0`.
- Cada red se consulta en dos tramos por la misma razón: primero lo que se lee con los permisos que ya hay para publicar, después lo que exige permiso de estadísticas. Si el segundo falla, el primero sobrevive. En Instagram se piden `reach,saved,shares,views` juntas y, si Meta rechaza el conjunto, se reintenta sin `views` — decidirlo por el tipo de pieza era falso: un carrusel del feed devolvió 746 reproducciones.
- **Facebook da menos que Instagram**, y esto está comprobado contra la API con una entrada real. Las reacciones y comentarios se cuentan por `summary` y piden `pages_read_user_content`, distinto de `pages_read_engagement`: ése deja publicar y leer la página, no leer lo que la gente hace en cada entrada. Las métricas de la entrada piden `read_insights`, y sin él **no dan error**: devuelven listas vacías, que es el peor modo de fallar porque parece que la publicación no tuvo movimiento. Y `post_impressions`/`post_impressions_unique` ya no son métricas válidas en ninguna versión viva, así que **no hay alcance de entrada en Facebook** — se queda en nulo y sale como «—»; no es un permiso que falte, es que no existe.
- Se cuentan **reacciones** y no «me gusta»: en Facebook alguien puede poner «me enorgullece» o «me encanta», y contar sólo los pulgares dejaría fuera parte de la reacción real.
- Los números **no se traen al abrir la pestaña**, sólo con el botón: cada apertura serían tantas llamadas a Meta como publicaciones tenga el mes. Y si una consulta no devuelve nada, no se guarda: pisar los números buenos de ayer con nulos de hoy perdería el único dato que había.
- Lo marcado a mano como publicado no tiene `externalPostId`, así que no hay a quién preguntarle. La pantalla lo dice en esa fila en vez de disimularlo con guiones, porque es una consecuencia de cómo se publicó, no un fallo.
- El calendario tiene dos caras según el ancho. En el celular una casilla mide unos 49 px: no cabe texto, así que lleva sólo el número y **un punto por pieza**, y se toca el día para verlo desplegado a lo ancho debajo del mes, con títulos, horas y redes. De tablet para arriba salen las fichas completas dentro de cada casilla. Meter fichas con título en 49 px era la causa de que se sintiera apretado e ilegible.
- El arrastre usa **dos sensores**, no uno. Con `PointerSensor` un deslizamiento de 6 px con el dedo arrancaba un arrastre en vez de un toque, así que abrir una pieza en el celular fallaba la mitad de las veces. Ahora el ratón arrastra al instante y el dedo tiene que mantenerse apoyado (220 ms), con lo que un toque sigue siendo un toque.
- Las fichas llevan `touch-manipulation` en el celular y `touch-none` sólo de tablet para arriba. Con `touch-none` en todas partes, apoyar el dedo en una ficha impedía desplazar la página, y en un mes lleno casi todo es ficha; el arrastre táctil no lo necesita porque se activa manteniendo pulsado.
- Hay un `DragOverlay`: una copia de la ficha que sigue al puntero. Sin ella el original sólo se atenuaba y nada se movía, así que en el celular el arrastre parecía roto aunque estuviera funcionando.
