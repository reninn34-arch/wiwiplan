...
- **Facebook da bastante menos que Instagram**, y esto está comprobado contra la API, no leído en la documentación: `likes.summary` y `comments.summary` se rechazan con error 10 aunque el token tenga `pages_read_engagement` —piden además `pages_read_user_content`, que no hace falta para publicar—, y `post_impressions`/`post_impressions_unique` ya no son métricas válidas, así que **no hay alcance de entrada disponible**. Se queda en nulo a propósito en vez de intentarlo y llenar los registros de avisos en cada actualización. Lo que sí vale: `post_reactions_by_type_total` —desglosado por tipo, hay que sumarlo— y `post_video_views_organic`.
